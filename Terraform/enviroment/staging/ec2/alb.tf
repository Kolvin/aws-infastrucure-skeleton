...
resource "aws_alb_listener" "ssl" {
  ...
  default_action {
    target_group_arn = "${aws_alb_target_group.staging-app-tg.arn}"
    type             = "forward"
  }
...
// ---------------- STAGING APP ----------------
resource "aws_alb_target_group" "staging-app-tg" {
  name                 = "staging-app-tg"
  port                 = "${var.backend_port}"
...
resource "aws_lb_target_group_attachment" "staging-app-instances" {
  count            = "${var.instance_count}"
  target_group_arn = "${aws_alb_target_group.staging-app-tg.arn}"
  target_id        = "${element(aws_instance.staging-app.*.id, count.index)}"
  port             = 80

  depends_on = ["aws_alb_target_group.staging-app-tg"]
}

// Staging Domain Listener Rule
resource "aws_lb_listener_rule" "staging_app_host_routing" {
